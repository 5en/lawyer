...
public class EventHelper {
    ...
    public static void checkedChanged(IPresenter li, RadioGroup... views) {
        if (views == null || views.length == 0) return;
        for (RadioGroup v : views)
            v.setOnCheckedChangeListener((RadioGroup.OnCheckedChangeListener) li);
    }
    ...
    public static void checkedChanged(RadioGroup.OnCheckedChangeListener li, RadioGroup... views) {
        if (views == null || views.length == 0) return;
        for (RadioGroup v : views) v.setOnCheckedChangeListener(li);
    }
}
