 ...
 */
package com.yanglaoClient.mvp.util.config;

import android.content.Context;


import com.yanglaoClient.mvp.util.core.ReflectUtils;
...
public class PropertiesConfig implements IConfig {
	...
	private static final String ASSET_CONFIGS = "/assets/config.properties";
	...
	public void loadConfig() {
		loadConfig(PropertiesConfig.class, ASSET_CONFIGS);
		setBoolean(LOADFLAG, true);
	}

	private void loadConfig(Class<?> cls, String configPath) {
		Properties props = new Properties();
		InputStream in = cls.getResourceAsStream(configPath);
		try {
			if (in != null) {
				props.load(in);
